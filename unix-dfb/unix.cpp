...
IDirectFBEventBuffer *joystickbuffer[MAX_JOYSTICKS] =
    { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
IDirectFBInputDevice *keyboard = NULL;
IDirectFBEventBuffer *keyboardbuffer = NULL;



#endif

#define SNES_TL2_MASK (1 << 32)
#define SNES_TR2_MASK (1 << 31)
...
void S9xParsePortConfig (ConfigFile &conf, int pass)
{
	...
	{
		keymaps.push_back(strpair_t("RIGHT",  "Joypad1 Right"));
		keymaps.push_back(strpair_t("LEFT",   "Joypad1 Left"));
		keymaps.push_back(strpair_t("DOWN",   "Joypad1 Down"));
		keymaps.push_back(strpair_t("UP",     "Joypad1 Up"));
		keymaps.push_back(strpair_t("A",      "Joypad1 A"));
		keymaps.push_back(strpair_t("B",      "Joypad1 B"));
		keymaps.push_back(strpair_t("X",      "Joypad1 X"));
		keymaps.push_back(strpair_t("Y",      "Joypad1 Y"));
		keymaps.push_back(strpair_t("TL",     "Joypad1 L"));
		keymaps.push_back(strpair_t("TR",     "Joypad1 R"));
		keymaps.push_back(strpair_t("START",  "Joypad1 Start"));
		keymaps.push_back(strpair_t("SELECT", "Joypad1 Select"));
	}
...
{
         if (strcmp(n, "RIGHT") == 0)  return S9xMapButton(SNES_RIGHT_MASK, *cmd, false);
	else if (strcmp(n, "LEFT") == 0)   return S9xMapButton(SNES_LEFT_MASK, *cmd, false);
	else if (strcmp(n, "DOWN") == 0)   return S9xMapButton(SNES_DOWN_MASK, *cmd, false);
	else if (strcmp(n, "UP") == 0)     return S9xMapButton(SNES_UP_MASK, *cmd, false);
	else if (strcmp(n, "A") == 0)      return S9xMapButton(SNES_A_MASK, *cmd, false);
	else if (strcmp(n, "B") == 0)      return S9xMapButton(SNES_B_MASK, *cmd, false);
	else if (strcmp(n, "X") == 0)      return S9xMapButton(SNES_X_MASK, *cmd, false);
	else if (strcmp(n, "Y") == 0)      return S9xMapButton(SNES_Y_MASK, *cmd, false);
	else if (strcmp(n, "TL") == 0)     return S9xMapButton(SNES_TL_MASK, *cmd, false);
	else if (strcmp(n, "TR") == 0)     return S9xMapButton(SNES_TR_MASK, *cmd, false);
	else if (strcmp(n, "START") == 0)  return S9xMapButton(SNES_START_MASK, *cmd, false);
	else if (strcmp(n, "SELECT") == 0) return S9xMapButton(SNES_SELECT_MASK, *cmd, false);
...
DFBEnumerationResult enum_devices_callback(unsigned int id, 
    ...
    if (desc.type & DIDTF_KEYBOARD) {
        DFBCHECK(dfb->GetInputDevice(dfb, id, &keyboard));
        DFBCHECK(keyboard->CreateEventBuffer(keyboard, &keyboardbuffer));
        fprintf(stderr, "Found 1 Keyboard with id: %d\n", id);
    } else if (desc.type & DIDTF_JOYSTICK) {
        int j = nr_joysticks;
        DFBCHECK(dfb->GetInputDevice(dfb, id, &joystick[j]));
        DFBCHECK(joystick[j]->CreateEventBuffer(joystick[j], &joystickbuffer[j]));
        nr_joysticks++;
        fprintf(stderr, "Found Joystick with id: %d\n", id);
    }
    return DFENUM_OK;
}

void S9xInitInputDevices (int argc, char *argv[])
{
